...
def init_db():
    ...
    if conn.execute("SELECT COUNT(*) c FROM applications").fetchone()["c"] == 0:
        seed = [
            ("Долг по ЖКХ, мать с двумя детьми","После развода накопились коммунальные долги. Боюсь, что зимой отключат свет и воду.",120000,48200,312),
            ("Микрозаймы после сокращения","Попал под сокращение, перекрыл зарплату займами. Проценты съедают всё, что нахожу.",60000,17900,128),
            ("Штраф — под угрозой автомобиль","Машина — единственный способ работать. Если её заберут за долг, останусь без дохода.",95000,91400,540),
            ("Кредитка после больницы","Три месяца без работы из-за операции. Долг по карте рос каждый день.",75000,32500,201),
        ]
        for t,s,a,r,sup in seed:
            conn.execute("INSERT INTO applications (title,story,amount,raised,supporters,status,demo) VALUES (?,?,?,?,?,'approved',1)",(t,s,a,r,sup))
        conn.commit()
